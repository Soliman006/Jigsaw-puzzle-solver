...
class SolverData:
    ...
    def sequentialsolve(self):
        sum_score = 0
        while len(self.memory)!=54:
            space = [len(self.memory)%6,len(self.memory)//6]
            x = space[0]
            y = space[1]
    ...
    def fullsolver_rows(self):
        self.setloctype(6,9)
        best_edge1,best_score1 = self.solvebestedge(3)
        self.hardReset()
        best_edge2,best_score2 = self.solvebestedge(1)
        self.hardReset()
        ...
        self.removepieces()
        best_edge_other,best_score_other = self.solvebestedge(1)
        self.hardReset()
        for i in best_edge_other:
            self.solution.append(i)
        
        self.setloctype(9,6)

        self.removepieces()
        best_edge_top,best_score_top = self.solvebestedge(0)
        self.hardReset()
        for i in best_edge_top:
            self.solution.append(i)
            
        self.removepieces()
        best_edge_bot,best_score_bot = self.solvebestedge(2)
        self.hardReset()
        for i in best_edge_bot:
            self.solution.append(i)
            
        for i in range(self.data.puzzle_rows-1):
            self.solution[i].space = [0,self.data.puzzle_rows-1-i]
            self.solution[i].options[self.solution[i].choice].rotation = 3
            index =i+self.y_limit-1
            self.solution[index].space = [self.data.puzzle_columns-1,i]
            self.solution[index].options[self.solution[index].choice].rotation = 1
        for i in range(self.data.puzzle_columns-1):
            index = i+2*(self.data.puzzle_rows-1)+self.data.puzzle_columns-1
            self.solution[index].space = [self.data.puzzle_columns-1-i,self.data.puzzle_rows-1]
        ...
        best_row, best_row_score = self.solvebestrow(1)
        for i in best_row:
            self.solution.append(i)
        ...
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_row,best_row_score = self.solvebestrow(2)
        for i in best_row:
            self.solution.append(i)
        ...
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_row,best_row_score = self.solvebestrow(3)
        for i in best_row:
            self.solution.append(i)
        ...
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_row,best_row_score = self.solvebestrow(4)
        for i in best_row:
            self.solution.append(i)
    ...
    def fullsolver_columns(self):
        ...
        '''self.solveinterior_column1()
        self.solveinterior_column2()
        self.solveinterior_column3()
        self.solveinterior_column4()
        self.solveinterior_column5()
        self.solveinterior_column6()
        self.solveinterior_column7()'''
    
    def solveborder(self):
        self.setloctype(6,9)
        best_edge1,best_score1 = self.solvebestedge(3)
        self.hardReset()
        best_edge2,best_score2 = self.solvebestedge(1)
        ...
        for i in corners:
            ...
            for j in best_edge:
                ...
                if count == len(best_edge):
                    ...
                    if j.options[j.choice].score > 2 * edge_average:
                        best_score = 10
                else:
    ...
    def solveinterior_column2(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(2)
        for i in best_column:
            self.solution.append(i)
    
    def solveinterior_column3(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(3)
        for i in best_column:
            self.solution.append(i)
    
    def solveinterior_column4(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(4)
        for i in best_column:
            self.solution.append(i)
    
    def solveinterior_column5(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(5)
        for i in best_column:
            self.solution.append(i)
    
    def solveinterior_column6(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(6)
        for i in best_column:
            self.solution.append(i)
            
    def solveinterior_column7(self):
        self.hardReset()
        self.placecurrentsolution()
        self.flags.solvingborder = False
        best_column, best_column_score = self.solvebestcolumn(7)
        for i in best_column:
            self.solution.append(i)
    ...
    def solvebestedge(self,corner):
        sum_score = 0
        potential_edge = []
        while self.flags.solvable != 0:
            while not self.flags.cornerfound:
    ...
    def solveaborder(self):
        ...
        self.forward_border = list(self.memory)
        for i in range(len(self.memory)):
            sum_score = sum_score + self.memory[i].options[self.memory[i].choice].score
        print('score for the border from ',self.memory[0].space,'to ',self.memory[len(self.memory)-1].space,'is ',sum_score)
        ...
        self.backwards_border = list(self.memory)
        for i in range(len(self.memory)):
            sum_score = sum_score + self.memory[i].options[self.memory[i].choice].score
        print('score for the border from ',self.memory[0].space,'to ',self.memory[len(self.memory)-1].space,'is ',sum_score)
    ...
    def solvePath(self):
        ...
        while self.placed_pieces.min() == 0:
            if self.flags.solve_interior == 0:  # if we are not solving the interior yet
                if (len(self.processed_corners) + len(self.processed_edges)) == 0:  # if the border is complete
                    break
    ...
    def manualCompare2(self, piece1, side1, rotation1, piece2, side2, rotation2):
        """Command allowing the user to manually compare how well 2 pieces match."""
        img_processed_bgr = copy.deepcopy(self.data.img_processed_bgr)
        contour1 = self.data.processed_pieces[piece1][(side1 + rotation1) % 4]
        contour2 = self.data.processed_pieces[piece2][(side2 + rotation2) % 4]
        ...
        reference = process_contour(contour1)
        candidate = remove_duplicates(contour2)
        '''print(len(changed_contour))
        for i in changed_contour:
            print(i)'''
        #closest_dist = findeuclid_dist(reference,candidate)
        ...
        '''score_shape, score_colour, score_total = compareContours(
            contour1, contour2, colour_curve1, colour_curve2, colour_contour_xy1, colour_contour_xy2, self.data.av_length, self.settings)'''
        score_colour = compareContours(colour_curve1, colour_curve2, colour_contour_xy1, colour_contour_xy2, self.data.av_length, self.settings)
        score_shape = findeuclid_dist(reference,candidate)
        score_shape = findeuclid_dist(reference,candidate)
        score_total = 0.7*score_shape + 0.3*score_colour
        print("Comparing piece", piece1, "(side", side1, "rot", rotation1, ") with piece", piece2, "(side", side2,
              "rot", rotation2, "). ", f'Scores: shape {score_shape:.4f} colour {score_colour:.4f} total {score_total:.4f}')
        # bgr overlays
        cv2.polylines(img=img_processed_bgr, pts=[self.data.processed_pieces[piece1][(
